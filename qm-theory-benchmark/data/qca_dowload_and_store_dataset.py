...
client = FractalClient.from_file(verify=False)
ds = client.get_collection("Dataset", "OpenFF Theory Benchmarking Single Point Energies v1.0")

# for the sake of querying convenience listing out the keywords, methods and basis sets explicitly
keywords_list = [
    "default",
    "b3lyp-nl/dzvp",
...
for i, key in tqdm(enumerate(keywords_list)):
    print(key)
    records = ds.get_records(method=methods[i], basis=basis[i], keywords=key)
    ...
    if length_of_records == number_of_records and not isinstance(records, list):
        for index in tqdm(records.index):
            if records.loc[index].record.status == "COMPLETE":
                single_points_data[(index, methods[i], basis[i], key)] = records.loc[
                    index
                ].record.dict()
                single_points_data[(index, methods[i], basis[i], key)]["stdout"] = records.loc[
                    index
                ].record.get_stdout()
                single_points_data[(index, methods[i], basis[i], key)][
        ...
        for index in tqdm(records[1].index):
            if (
                ...
                single_points_data[(index, methods[i], basis[i], key)]["dispersion_stdout"] = (
                    records[0].loc[index].record.get_stdout()
                )

                single_points_data[(index, methods[i], basis[i], key)]["stdout"] = (
