...
def main(data_pickle):
    ...
    pdf = PdfPages("../outputs/torsions_chno_set_ccsd_cbs_ref.pdf")
    ...
    for i, index in enumerate(df_mp2.index):
        ...
        sym_atoms = list(set(offmol.hill_formula))
        no_integers = [x for x in sym_atoms if not (x.isdigit() or x[0] == '-' and x[1:].isdigit())]
        superset = ['C', 'H', 'N', 'O', 'S']
        if not no_integers <= superset:
            continue
    ...
    with open("../outputs/torsion_spe_chno_subset_analysis_scores.txt", "w") as f:
