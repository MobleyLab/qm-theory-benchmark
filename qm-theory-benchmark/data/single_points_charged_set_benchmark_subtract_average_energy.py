...
def main(data_pickle):
    ...
    pdf = PdfPages("../outputs/torsions_charged_set_subtract_average.pdf")
    ...
    for i, index in enumerate(df_mp2.index):
        ...
        energy_ave = np.average(energies_spe)
        relative_energies = [
            (x - energy_ave) * HARTREE_TO_KCALMOL for x in energies_spe
        ]
        ...
        for j, spec in enumerate(keywords_list):
            ...
            if j != len(keywords_list) - 1:
                ...
                energy_ave = np.average(energies_spe)
                energies = [(x - energy_ave ) * HARTREE_TO_KCALMOL for x in energies_spe]
                ...
                energies = (
                    np.array(df_mp2.loc[index, spec][0]["relative_energies_subtract_average"])
                    * HARTREE_TO_KCALMOL
                )
    ...
    with open("../outputs/torsion_spe_charged_subset_subtract_average.txt", "w") as f:
