...
def main(data_pickle):
    ...
    pdf = PdfPages("../outputs/torsions_neutral_set_ccsd_cbs_ref_b3lyp_single_basis.pdf")
    ...
    for i, index in enumerate(df_mp2.index):
        ...
        if offmol.total_charge.value_in_unit(unit.elementary_charge) != 0:
            continue
        elif offmol.total_charge.value_in_unit(unit.elementary_charge) == 0  and mapped_smiles.find('+') != -1:
            continue
    ...
    with open("../outputs/torsion_spe_neutral_subset_analysis_scores_b3lyp_single_basis.txt", "w") as f:
