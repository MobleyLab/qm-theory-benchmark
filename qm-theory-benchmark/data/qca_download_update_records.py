...
def main():
    # To update current pickle file with any new calculations or additional specs
    ...
    # new specification to download or old one to update
    specifications = ["MP2/aug-cc-pVTZ"]
    print(specifications)

    # Create a dataframe to store the torsiondrives data
    df = pd.read_pickle("./torsiondrive_data.pkl")
    for i, entry_index in enumerate(ds.df.index):
        for spec_name in specifications:
            ...
            df.at[entry_index + str(i), spec_name] = [td_dict]
